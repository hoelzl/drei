...
**Not yet wired.** No key binds an agent command, `run_editor` has no injection point for asynchronous deliveries, and nothing launches or speaks to a `hermes acp` child: the §C pump — a streaming port, the delivery/keystroke serialization, cancellation wiring, and the decision of which buffer a transcript binds to — is the next design record. Until it lands the subsystem is exercised by tests, not by users. See `docs/technical-debt.md`.
