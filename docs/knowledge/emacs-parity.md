...
| find-file replacing the buffer | wholesale replace of the single buffer (id unchanged); undo history cleared; kill ring preserved; mark cleared; point 0 | switches to a new per-file buffer; per-buffer undo history preserved | Drei is single-buffer for now; multi-buffer is a follow-up slice |
