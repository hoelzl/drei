...
Raw terminal frames are evidence but never the sole semantic oracle. A scenario records inputs, constraints, readiness, semantic observations, terminal observations, and outcome. [Design 0006](../agent/design/0006-session-genesis.md) names the initial state explicitly: equivalent `SessionGenesisV1`, equivalent injected effect outcomes, and the same ordered subsequent commands and external deliveries must produce equivalent semantic evidence. The current event tuple alone is not yet a persisted replay format.
