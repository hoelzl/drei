...
- Shipped-terminal evidence: `uv --no-config run pytest tests/termverify -q` (ConPTY, Windows; skipped on other platforms in TermVerify 0.1.x), or run `uv --no-config run drei` in a real TTY (writes `DREI:READY`, emits OSC 7791 readiness markers, and exits cleanly on `C-x C-c` — answering `y` to `Modified buffers exist; exit anyway?` if you typed anything, since a modified buffer is no longer discarded silently; `C-g` is `keyboard-quit` and does not exit).
