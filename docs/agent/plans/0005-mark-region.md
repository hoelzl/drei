...
- `KillRegion`: requires a set mark; kills `text[min(point,mark):max(point,mark)]` as ONE ring entry (chain head push, not append — Emacs pushes region kills as new entries; kill-line append chain is broken first). Point moves to `min(point, mark)` (the kill position). Mark cleared. Event: `TextKilled(text, before, after, direction)` reused with `direction` = `"forward"` (mark > point) or `"backward"` (mark < point); mark == point kills nothing (empty region → silent no-op, deviation from Emacs's "The mark is not set now, or the region is empty" error — same no-echo-error rationale).
- `CopyRegionAsKill`: same region; pushes text into ring; buffer text and point unchanged; mark cleared. Event: `RegionCopied(text)` (new event — NOT text-changing, so it must NOT set `modified`; the modified-flag property test needs an explicit arm for it).
- `ExchangePointAndMark`: requires a set mark; swaps point and mark; mark stays set. Event: `PointMoved`? No — new event `PointMarkExchanged(old_point, old_mark)`? Simplest: reuse `PointMoved(0, actual)`? PointMoved carries (goal, actual) — exchange isn't a goal-directed move. New event `MarkExchanged(point_before, mark_before)`. No-op without a mark (silent, deviation).
...
- Events must keep the transcript the oracle: mark state is derivable from `MarkSet`/`TextKilled`/`RegionCopied`/`MarkExchanged`/clearing events. A cleared mark needs NO event of its own (derivable: mark cleared by C-g/kill-region/copy — positional). But then mark==None vs mark-set must be reconstructible — the fold tracks it. Document the fold rule in the plan.
...
- Region kill/copy/exchange pinned by the pinned-Emacs differential; deactivation/no-op deviations in the registry.
- Kill chain interplay: `C-k C-k C-@ C-w` — the region kill opens a NEW entry even after chained line kills (focused test).
- Transcript mark-fold coherence property.
...
- Adding a field to `BufferValue` touches every `replace(...)` call site? No — dataclass default keeps call sites unchanged; validation lives in `__post_init__`.
