...
- **ConPTY/TermVerify `C-@` probe (this session):** the raw NUL byte (`TextInput("\x00")`) survives ConPTY and reaches the child. But `termverify.key/v1` has NO chord for `C-@`/`C-SPC` — `("Control", "@")` is not canonical and `("Control", "Space")` fails closed in `key-encoding/v1` (adapter refuses to send). **Implementation finding (supersedes):** NUL is undeliverable on the Windows console anyway — `msvcrt.getwch` treats it as an extended-key prefix and swallows the following byte as a scan code (verified live: NUL+`Z` consumes `Z`). `_read_key_windows` now pair-consumes the prefix; the TermVerify scenario is a documented skip; region commands are proven in-process through the same `run_editor` byte loop; the constraint is a registry deviation.
...
- Keys: `C-@` is `0x00` (NUL byte) — `decode_key` maps `\x00` → `C-@`; `C-SPC` sends the same byte on terminals. `C-w` = `0x17`, `C-x C-x` via the existing prefix table, `M-w` = ESC+w via `assemble_meta`/`_META_KEYS` (already built). (`assemble_meta` was replaced by `KeyAssembler` in cluster B of review 0001 — `ESC`+letter → `M-<letter>` is unchanged, and `\x00` no longer decodes to `C-@` on the Windows console; see plan 0004's Scope note and the parity registry.)
...
5. TermVerify scenario: C-@ is undeliverable through ConPTY on Windows (msvcrt extended-key prefix — see the probe note) → the scenario is a documented skip; `C-@ C-f C-f C-w C-y`, `M-w`, and `C-x C-x` are proven in-process through the same `run_editor` byte loop. `M-w` copy — ESC-swallow caveat applies (in-process proof).
