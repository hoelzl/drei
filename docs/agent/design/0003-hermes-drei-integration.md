...
**Extended by:** `0004-agent-buffer-identity.md` (defines the *agent buffer* this record names in §Vocabulary but never binds) and `0005-acp-pump.md` (places the §C boundary and amends §A.1 — see below).
...
1. **Subprocess effect port.** Launch/monitor/terminate a child process; deliver its stdout/stderr lines and exit status into the session as immutable external-input events. *Why:* ACP is stdio JSON-RPC to a subprocess. *Verify:* in-process with a scripted fake child emitting known bytes; property: delivered events are ordered and replayable. **Amended (2026-07-25, `0005-acp-pump.md` D1):** plan 0008 deliberately shipped a narrower *blocking* run-to-completion port and deferred streaming, and this text was never updated — read as shipped code it has been wrong for five slices. A.1 is the blocking port; the long-lived streaming port that can hold an ACP conversation is 0005's D1.
...
- **How much of the transcript is editable.** Whether the human may edit the agent buffer mid-session (and how that reconciles with subsequent `session/update` folds) is undecided. *Trigger:* slice 3 (read-only/generated buffers) should resolve the default; revisit only if a real workflow demands mid-stream edits. (`0004-agent-buffer-identity.md` D6 deliberately leaves this open: it decides *where* a transcript goes, not who may edit it, and supplies the buffer-kind hook §A.3 needs.)
